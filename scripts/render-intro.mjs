...
// summon (see src/viz/PlayerHost.tsx + src/viz/WarRoom.tsx). Playing the
...
//   2. Wrap src/viz/compositions/Intro.tsx in a <Composition> inside a Remotion
//      entry (e.g. src/viz/remotion-root.tsx) registered via registerRoot().
//   3. bundle() that entry, selectComposition('Intro'), then renderMedia({
//        codec: 'vp9', outputLocation: 'src/assets/intro.webm', ...
//      }).
//   4. Commit the rendered src/assets/intro.webm and have WarRoom prefer the
...
console.log('[render-intro] on first summon (src/viz/PlayerHost.tsx). No @remotion/renderer.');
