...
fn main() {
    ...
    // OUTSIDE the repo (so real local data never lands in the tree). Override the
    // destination with WARDEN_RADAR_PROBE_OUT.
    let json = serde_json::to_string_pretty(&state).expect("serialize radar state");
    let out = std::env::var_os("WARDEN_RADAR_PROBE_OUT")
        .map(PathBuf::from)
        .unwrap_or_else(|| std::env::temp_dir().join("warden_radar_probe.json"));
    if let Some(parent) = out.parent() {
        std::fs::create_dir_all(parent).expect("create probe output parent");
    }
    std::fs::write(&out, &json).expect("write radar probe output");
